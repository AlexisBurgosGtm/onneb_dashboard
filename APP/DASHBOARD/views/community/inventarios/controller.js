...
async function getInventarios(idcontenedor){
    ...
    container.innerHTML = '<h1>Cargando datos..</h1>'
    let str = ``;

    axios.get('/reportscommunity/inventarios?token=' + GlobalToken)
    .then((response) => {
        const data = response.data;        
        data.recordset.map((rows)=>{
            str += `<tr>
                        ...
                        <td class='text-warning'><b>${rows.SALDO}</b>
                        <br>
...
async function getInventarioProducto(idcontenedor,idfiltro){
 ...
 let filtro = document.getElementById(idfiltro).value;
    ...
    axios.get('/reportscommunity/inventarioproducto?token=' + GlobalToken + '&filtro=' + filtro)
    .then((response) => {
        const data = response.data;        
        data.recordset.map((rows)=>{
            str += `<tr>
...
async function getDataProducto(empnit,codprod){
    ...
    let str ='';
    axios.get(`/reportscommunity/preciosproducto?token=${GlobalToken}&empnit=${empnit}&codprod=${codprod}`)
    .then((response) => {
        const data = response.data;        
        data.recordset.map((rows)=>{
            str += `<tr>
